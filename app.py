...
def authentication_page(df_movies, cosine_sim):
    ...
    with col1:
        # Sử dụng type="secondary" để có màu tối/viền nhạt
...
def plot_recommendation_comparison(df_results, recommendation_type, movie_name=None):
    ...
    # 2. Tạo màu sắc riêng cho mỗi phim (Dark Theme Palette)
    ...
    bars = ax.bar(df_plot['Tên phim'], df_plot[score_col], 
                   color=colors, edgecolor='#E0E0E0', alpha=0.9) # Viền nhạt
...
def get_zero_click_recommendations(df_movies, selected_genres, num_recommendations=15):
    ...
    # Lấy thông tin Năm phát hành để hiển thị trên Card
...
def display_movie_grid(df_results, score_column, num_cols=5):
    ...
    for index, row in df_results.iterrows():
        ...
        year = int(row.get('Năm phát hành', 'N/A'))
        
        # Placeholder Image URL
        # Sử dụng API Placeholder đơn giản và chuyển tên phim thành chuỗi URL an toàn
        placeholder_text = title.replace(' ', '+')
        placeholder_url = f"https://placehold.co/180x250/1A1D20/FF4500?text={placeholder_text}"
        ...
        <div class="movie-card">
            <div class="movie-poster" style="background-image: url('{placeholder_url}'); background-size: cover; background-position: center; border-bottom: 3px solid #FF450080;">
                <span style="background-color: #0F1113A0; padding: 5px; border-radius: 5px;">{year}</span>
            </div>
