    ...
    if 'df_users' not in st.session_state:
        try:
            ...
            df_users['ID'] = pd.to_numeric(df_users['ID'], errors='coerce')
             ...
             # Tạo DataFrame rỗng nếu không tìm thấy file
...
def get_recommendations(username, df_movies, num_recommendations=10):
    ...
    if user_row.empty: return pd.DataFrame() # Kiểm tra an toàn

    try:
        # Xử lý chuỗi list phim (ví dụ: "['Phim 1', 'Phim 2']")
        watched_movies_str = user_row['5 phim coi gần nhất'].iloc[0]
        watched_list = ast.literal_eval(watched_movies_str)
    except (ValueError, SyntaxError, IndexError):
        # Fallback nếu chuỗi không phải định dạng list
        watched_movies_str = user_row['5 phim coi gần nhất'].iloc[0]
    ...
    # Sắp xếp ưu tiên: Thể loại giống nhiều nhất, sau đó là Độ phổ biến cao nhất
...
def plot_genre_popularity(movie_name, top_movies, df_movies, is_user_based=False):
    ...
    # Đảm bảo top_movies không rỗng
    if top_movies.empty:
        st.warning("Không có dữ liệu phim đề xuất để vẽ biểu đồ.")
        return
    ...
    if is_user_based:
        ...
        # Kiểm tra user_row lần nữa, dù đã kiểm tra ở hàm gọi
        ...
        # Lấy phim gốc (được chọn) và phim đề xuất
    ...
    for index, row in combined_df.iterrows():
        # Bỏ qua hàng có thể loại rỗng
        if not row['Thể loại phim']: continue
    ...
    if not genres_data:
        st.warning("Không tìm thấy thể loại phim để vẽ biểu đồ.")
        return

    df_plot = pd.DataFrame(genres_data)
    genre_avg_pop = df_plot.groupby('Thể loại')['Độ phổ biến'].mean().reset_index()
    
    # Chỉ lấy 7 thể loại phổ biến nhất để biểu đồ dễ đọc
...
def main_page(df_movies, cosine_sim):
    ...
    if menu_choice == 'Đề xuất theo Tên Phim':
        ...
        if st.button("Tìm Đề Xuất", key="find_sim"):
            # Vô hiệu hóa checkbox khi nhấn nút để tránh lỗi logic
            if 'plot_sim_check' in st.session_state:
                 del st.session_state['plot_sim_check']
            ...
            if not result.empty:
                ...
                st.checkbox("📊 Hiển thị Biểu đồ so sánh Thể loại", key="plot_sim_check")
                
                # Kiểm tra sau khi tạo checkbox
                if st.session_state.get('plot_sim_check'):
        ...
        if st.button("Tìm Đề Xuất Hồ Sơ", key="find_profile"):
            # Vô hiệu hóa checkbox khi nhấn nút để tránh lỗi logic
            if 'plot_profile_check' in st.session_state:
                 del st.session_state['plot_profile_check']
            ...
            if not recommendations.empty:
                ...
                st.checkbox("📊 Hiển thị Biểu đồ so sánh Thể loại", key="plot_profile_check")
                
                # Kiểm tra sau khi tạo checkbox
                if st.session_state.get('plot_profile_check'):
